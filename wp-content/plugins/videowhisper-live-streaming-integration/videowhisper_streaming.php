...
HTMLCODE;
			...
			if ($editPost)
			{
				...
				if (VWliveStreaming::inList($userkeys, $options['uploadPicture']))
				{
					...
					$extraRows .= '<tr><td>Picture</td><td><input type="file" name="uploadPicture" id="uploadPicture"><BR>Update channel picture.</td></tr>';


					$value=get_post_meta( $editPost, 'showImage', true );

					$extraRows .= '<tr><td>Show Picture</td><td><select name="showImage" id="showImage">';
...
HTMLCODE;
		...
		function transcodeStream($stream, $required=0, $detect=2, $convert=1)
		{
			...
			if ($detect == 2 || ($detect == 1 && !$videoCodec))
			{
				...
				$cmd = 'timeout -s KILL 10 ' . $options['ffmpegPath'] .' -y -i "' . $rtmpAddressViewI .'/'. $stream . '" 2>&1 ';
				$info = shell_exec($cmd);
...
HTMLCODE;
		...
		function videowhisper_mpeg($atts)
		{
			...
			if (is_single())
				if (get_post_type( $postID = get_the_ID() ) == $options['custom_post']) $stream = get_the_title(get_the_ID());

				$atts = shortcode_atts(array('channel' => $stream, 'width' => '480px', 'height' => '360px'), $atts, 'videowhisper_mpeg');
			...
			if ($streamName)
			{
				...
				$htmlCode = <<<HTMLCODE
<!--MPEG:$postID:$stream-->
<video id="videowhisper_mpeg_$stream" class="videowhisper_htmlvideo" width="$width" height="$height" data-dashjs-player autobuffer autoplay loop controls="true" poster="$thumbUrl" src="$streamURL">
    <div class="fallback" style="display:none">
    <IMG SRC="$thumbUrl">
	    <p>HTML5 MPEG Dash capable browser (i.e. Chrome) is required to open this live stream: $streamURL</p>
	</div>
</video>
<br>Please access using a PC browser with Flash plugin for best latency, full interface and interactions. Only plain live stream is available due to mobile browser limitations. Transcoding and HTTP based delivery technology involve high latency (may take dozens of seconds for stream to become available).
<br>
HTMLCODE;
			}
		...
		function flash_video($stream, $width = "100%", $height = '360px')
		{

			$stream = sanitize_file_name($stream);

			$swfurl = plugin_dir_url(__FILE__) . "ls/live_video.swf?ssl=1&n=" . urlencode($stream);
			$swfurl .= "&prefix=" . urlencode(admin_url() . 'admin-ajax.php?action=vwls&task=');
			$swfurl .= '&extension='.urlencode('_none_');
			$swfurl .= '&ws_res=' . urlencode( plugin_dir_url(__FILE__) . 'ls/');
...
HTMLCODE;
		...
		function videowhisper_video($atts)
		{
			...
			$htmlCode .= "<!--VideoWhisper-Agent-Video:$agent|A:$Android|I:$iOS|S:$Safari|F:$Firefox-->";
...
HTMLCODE;
			...
			return VWliveStreaming::flash_video($stream, $width, $height) . $afterCode;
		...
		function videowhisper_webrtc_broadcast($atts)
		{
			...
			$streamQuery = $stream . '?channelID=' . $postID . '&userID=' . urlencode($current_user->ID ) . '&key=' . urlencode($keyBroadcast);
			...
			if ($Firefox)
			{
				return "WebRTC broadcast with current settings is not supported in Firefox. Try opening this URL in Chrome or Safari!";
			}
...
HTMLCODE;
		...
		function videowhisper_webrtc_playback($atts)
		{
			...
			$streamQuery = $stream . '?channelID=' . $postID . '&userID=' . urlencode($userID) . '&key=' . urlencode($keyPlayback);
			...
			if ($Safari)
			{
				$htmlCode .=  "WebRTC playback is not currently fully supported for Safari. If live video does not play or freezes, try opening this URL in Chrome or Firefox!<BR>";
			}
			...
			$htmlCode .= <<<HTMLCODE
	WebRTC Playback: $stream
		<div class="videowhisper-webrtc-video">
		<video id="remoteVideo" autoplay style="width:$width; height:$height"></video>
		</div>
...
HTMLCODE;
		...
		function vwls_channels() //list channels
			...
			if ($options['postChannels']) //channel posts enabled
				...
				else echo "No channels match current selection. Channels get listed after being broadcast or configured as events, with snapshot/picture.";
...
Software</a>.</p></div>';
		...
		function channel_page($content)
		{
			...
			if ($options['broadcasterRedirect'] && !array_key_exists( 'broadcast' , $wp_query->query_vars )) //don't redirect from broadcast page
				{
...
<ul>
  ...
  Displays video only interface. If iOS is detected it shows HLS instead. Set html5=&quot;always&quot; to force html5 video.</li>
 ...
 <li><h4>[videowhisper_mpeg channel=&quot;Channel Name&quot; width=&quot;480px&quot; height=&quot;360px&quot;]</h4>
  Displays HTML5 MPEG DASH video interface. Shows istead of watch and video interfaces if Android is detected. Stream must be published in compatible format (H264,AAC) or transcoding must be enabled and active for stream to show.</li>
  <li>
  ...
  <?php
		...
		function adminOptionsDefault()
		{
			...
			return array(
				...
				'ffmpegTranscode' => '-analyzeduration 0 -c:v copy -c:a libfdk_aac -b:a 96',
				'ffmpegTimeout' => '60',
...
Editor = 1
				...
				'parameters' => '&bufferLive=1&bufferFull=1&showCredit=1&disconnectOnTimeout=1&offlineMessage=Channel+Offline&disableVideo=0&fillWindow=0&adsTimeout=15000&externalInterval=360000&statusInterval=90000&loaderProgress=1',
				'parametersBroadcaster' => '&bufferLive=2&bufferFull=2&showCamSettings=1&advancedCamSettings=1&configureSource=1&generateSnapshots=1&snapshotsTime=60000&room_limit=500&showTimer=1&showCredit=1&disconnectOnTimeout=1&externalInterval=360000&statusInterval=30000&loaderProgress=1&selectCam=1&selectMic=1',
...
<h2 class="nav-tab-wrapper">
   ...
   <a href="admin.php?page=live-streaming&tab=watch-params" class="nav-tab <?php echo $active_tab=='watch-params'?'nav-tab-active':'';?>">Watch Params</a>
    ...
    <a href="admin.php?page=live-streaming&tab=hls" class="nav-tab <?php echo $active_tab=='hls'?'nav-tab-active':'';?>">Transcoding</a>
...
<br>Only works for RTMP based clients (including external players when RTMP Session Control is setup). Does not work for HTML5 based mobile streaming.
...
?>
<h3>WebRTC</h3>
WebRTC can be used to broadcast and playback live video in HTML5 browsers. WebRTC is a new real time video communication technology under development and with specific requirements and limitations.
...
<select name="webrtc" id="webrtc">
  ...
  <option value="2" <?php echo $options['webrtc']=='2'?"selected":""?>>Preferred</option>
</select>
...
<select name="webrtcVideoCodec" id="webrtcVideoCodec">
  <option value="42e01f" <?php echo $options['webrtcVideoCodec']=='42e01f'?"selected":""?>>H.264</option>
 <!-- 
...
<UL>
	<LI>Chrome: Supports broadcast and playback.</LI>
	<LI>Safari: Supports broadcast. Playback only when published from Chrome. <LI>
	<LI>Firefox: Supports playback over UDP.</LI>
</UL>
Other WebRTC Limitations:
<UL>
	<LI>Advanced interactions specific to VideoWhisper Flash apps (like chat/kick) are not available. Only plain broadcast/playback is possible. TODO: HTML5 chat.</LI>
	<LI>Audio is available only when codecs match. TODO:Transcoding for RTP/RTMP streams.</LI>
</UL>
...
<?php
			...
			case 'hls':
?>
<h3>Transcoding, HLS, MPEG DASH</h3>
Configure transcoding and HTML5 based HLS, MPEG DASH delivery. HTTP Live Streaming is a great option for streaming to mobile browsers.
...
<BR>Ex: http://localhost:1935/videowhisper-x/ works when publishing to rtmp://localhost/videowhisper-x .
...
<BR>Show MPEG Dash for certain types of devices.
...
<BR>Redirect broadcaster when accessing own channel page to dashboard or broadcasting interface instead of watch interface.
	...
	<?php
	 ...
	 <?php
		...
		function streamSnapshot($stream, $ipcam = false)
		{
			...
			$cmd = 'timeout -s KILL 10 ' . $options['ffmpegPath'] . " -f image2 -vframes 1 \"$filename\" -y -i \"" . $rtmpAddressView ."/". $stream . "\" >&$log_file & ";
			...
			case 'rtmp_status':
				...
				if (is_array($rtpsessions))
					foreach ($rtpsessions as $rtpsession)
					{
						...
						if ($channelID = (int) $streamQuery['channelID'] )
						{
							$post = get_post($channelID);
						...
						if ($rtpsession['streamPublish'] == 'true' && $userID && !$disconnect) //WebRTC broadcaster session
							...
							$keyBroadcast = md5('vw' . $options['webKey'] . $userID  . $channelID);
